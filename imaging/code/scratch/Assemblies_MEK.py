...
import Miniscope
from Miniscope import open_minian, get_transient_timestamps
import util
...
def find_assemblies(neural_data, method='ica', nullhyp='mp',
    ...
    spiking, _, bool_arr = get_transient_timestamps(neural_data,
                                                    thresh_type='nope',do_zscore=True, std_thresh=5)
    if use_bool:
        actmat = bool_arr
    else:
    ...
    if compute_activity:
        ...
        if plot:
            ...
            #fig, axs = plot_assemblies_select(activations, sorted_spiking,colors=sorted_colors)
            fig, axs = plot_assemblies(activations, sorted_spiking,colors=sorted_colors)
    else:
        activations = None
...
def plot_assemblies_select(assembly_act, spiking, do_zscore=True, colors=None):
    """
    Plots assembly activations with S overlaid.
    ...
    for n, (ax, act, spikes, c) in \
        ...
        #print(act)
        print(max(act[0,:]))
        #act = [act[3,:],act[4,:],act[5,:]]
        #print(act)
        #uniq_colors = [uniq_colors[3],uniq_colors[4],uniq_colors[5]]
        for activation, assembly_color in zip(act, uniq_colors):    
            #print(type(activation))
            #print(assembly_color)
            #print(activation)
            ax[1].plot(activation, color=assembly_color, alpha=0.7,linewidth=2)
        #ax2 = ax.twinx()
        #ax[0].invert_yaxis()
        ...
        #print(len(c))
        numCells=20
        ax[0].eventplot(spikes, colors=c)
        ax[1].set_ylim(bottom=0)
        ax[0].set_ylim(bottom=30)
        ax[1].set_ylabel('Ensemble activation [a.u.]')
        ax[0].set_xlabel('Time [frame number]')
        ax[1].set_xlabel('Time [frame number]')
        ax[0].set_ylabel('Neurons')
        #ax[0].set_yticks([0, numCells])
        #ax[0].set_yticks([0, numCells])
        #ax.set_xlim(10000,15000)
        startylim = 18000
        endylim=25000
        #ax[0].set_xlim(startylim,endylim)
        #ax[1].set_xlim(startylim,endylim)
        fig.tight_layout()
